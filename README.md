...
一个面向 AstrBot 的多平台媒体 API 插件，专注于在群聊中为 LLM 提供图片/视频/音频能力。通用 MCP 服务器已移除，仅保留 AstrBot 场景。
...
此仓库仅作为 AstrBot 插件使用，无需单独启动 MCP 服务器。将项目作为插件放入 AstrBot 的 `plugins` 目录后重启 AstrBot 即可加载。
...
**参数（由 AstrBot 注入，不在 schema 中声明）**：

- `group_id`（可选）：群组ID，用于权限检查（由 AstrBot 根据上下文自动注入）
