> 本文由 [简悦 SimpRead](http://ksria.com/simpread/) 转码， 原文地址 [docs.astrbot.app](https://docs.astrbot.app/dev/star/plugin.html)
...
<table tabindex="0"><thead><tr><th>平台</th><th>At</th><th>Plain</th><th>Image</th><th>Record</th><th>Video</th><th>Reply</th><th>主动消息</th></tr></thead><tbody><tr><td>QQ 个人号 (aiocqhttp)</td><td>✅</td><td>✅</td><td>✅</td><td>✅</td><td>✅</td><td>✅</td><td>✅</td></tr><tr><td>Telegram</td><td>✅</td><td>✅</td><td>✅</td><td>✅</td><td>✅</td><td>✅</td><td>✅</td></tr><tr><td>QQ 官方接口</td><td>❌</td><td>✅</td><td>✅</td><td>❌</td><td>❌</td><td>❌</td><td>❌</td></tr><tr><td>飞书</td><td>✅</td><td>✅</td><td>✅</td><td>❌</td><td>❌</td><td>✅</td><td>✅</td></tr><tr><td>企业微信</td><td>❌</td><td>✅</td><td>✅</td><td>✅</td><td>❌</td><td>❌</td><td>❌</td></tr><tr><td>钉钉</td><td>❌</td><td>✅</td><td>✅</td><td>❌</td><td>❌</td><td>❌</td><td>❌</td></tr></tbody></table>
...
![](https://docs.astrbot.app/assets/image-5.r1malpg3.png)
...
* **Usage**删除会话中的某条对话；若 `conversation_id` 为 `None`，则删除当前对话。
* **Arguments**
  * `unified_msg_origin: str`
...
* **Usage**获取指定对话的完整对象；若不存在且 `create_if_not_exists=True` 则自动创建。
* **Arguments**
  * `unified_msg_origin: str`
  * `conversation_id: str`
...
* **Usage**更新对话的标题、历史记录或 persona_id。
* **Arguments**
  * `unified_msg_origin: str`
...
* **Usage**新建人格并立即写入数据库，成功后自动刷新本地缓存。
* **Arguments**
...
* **Returns**`Persona` – 新建后的人格对象
* **Raises**
  `ValueError` – 若 `persona_id` 已存在

##### `update_persona` 

* **Usage**更新现有人格的任意字段，并同步到数据库与缓存。
* **Arguments**
...
* **Usage**根据当前会话配置，获取应使用的默认人格（v3 格式）。若配置未指定或指定的人格不存在，则回退到 `DEFAULT_PERSONALITY`。
* **Arguments**
